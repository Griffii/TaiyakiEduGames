  ...
  <section class="flashcards-page" :class="pageClass">
    ...
    <div id="flashcard" class="flashcard-container fade" :class="cardContainerClass" @click="revealOrNextCard">
      ...
      <div class="english-container" v-if="!finished">
        <div class="english" :class="{ hidden: !revealed }" id="english-text">{{ currentCard?.english || "" }}</div>
      </div>
...
/**
 * FlashcardReview.vue — unified image resolver (public-assets for everything), premade + custom loaders.
 */
...
const mode = ref<"review" | "random" | "japanese">("review");
...
/** One canonical resolver: EVERYTHING defaults to public-assets unless explicit public path or absolute URL */
function toPublicUrlSmart(path?: string | null): string {
  ...
  // If someone saved with bucket prefix, strip it (we only support public-assets here)
  ...
  // Bare key → treat as a key inside public-assets (premade + custom both)
...
async function loadPremade(): Promise<Card[]> {
  ...
  if (selectedIds.value?.length) {
    ...
    mapped = mapped.filter((c) => idset.has(c.id));
  }
  if (mode.value === "random") mapped = shuffleCards(mapped);
  return mapped;
}
...
async function loadCustom(): Promise<Card[]> {
  ...
  // If 0 rows, some setups expose custom_deck_id instead; alias correctly (no inline comments!)
...
async function loadDeck() {
  ...
  // Try premade first (keep old behavior). If empty, fall back to custom.
  let premade: Card[] = [];
...
/* Hover on the button’s icon (scale + rotate) */
.settings-button:hover img,
.settings-button:focus-visible img {
  transform: scale(1.08) rotate(8deg);
}

/* Settings Menu (no hover transform on the panel) */
.settings-menu {
  position: absolute;
  top: 60px;
  right: 20px;
  ...
  width: 180px;
  text-align: center;
  /* No transform/rotate on hover for the menu itself */
}
...
.english { color: #1b3b6f; visibility: hidden; text-align: center; }
.english.hidden { visibility: hidden; }
.english:not(.hidden) { visibility: visible; }
